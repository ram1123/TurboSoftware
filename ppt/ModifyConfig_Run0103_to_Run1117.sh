...
awk -v OFS="\t" 'NR==FNR && NR==3 {a=$3} NR==FNR {next} FNR==6 	{$2=$2+a} {print}' fit_detail_$1.txt  OffsetFlip_EventBuilderVFAT_Oct2014_H2.conf.buffer1 > OffsetFlip_EventBuilderVFAT_Oct2014_H2.conf.buffer2
...
awk -v OFS="\t" 'NR==FNR && NR==6 {a=$3} NR==FNR {next} FNR==7 	{$3=$3+a} {print}' fit_detail_$1.txt OffsetFlip_EventBuilderVFAT_Oct2014_H2.conf.buffer2 > OffsetFlip_EventBuilderVFAT_Oct2014_H2.conf.buffer1
awk -v OFS="\t" 'NR==FNR && NR==7 {a=$3} NR==FNR {next} FNR==9 	{$3=$3+a} {print}' fit_detail_$1.txt OffsetFlip_EventBuilderVFAT_Oct2014_H2.conf.buffer1 > OffsetFlip_EventBuilderVFAT_Oct2014_H2.conf.buffer2
