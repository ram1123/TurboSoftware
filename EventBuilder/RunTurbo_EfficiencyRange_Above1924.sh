...
#  fi
  ...
  do
	...
	do
		cut2=$((cut + 5))
		awk 'NR==4{$(NF-9) = "'${cut}'"}1' OFS="\t" FS="\t" ConfigFiles/OffsetAndCoverage_Analyzer_N0V2014_H4_AboveRun1924.conf  > tmp1.conf
		awk 'NR==4{$(NF-8) = "'${cut2}'"}1' OFS="\t" FS="\t" tmp1.conf > tmp2.conf
		awk 'NR==5{$(NF-9) = "'${cut}'"}1' OFS="\t" FS="\t"  tmp2.conf > tmp1.conf
		awk 'NR==5{$(NF-8) = "'${cut2}'"}1' OFS="\t" FS="\t" tmp1.conf > tmp2.conf
		awk 'NR==6{$(NF-9) = "'${cut}'"}1' OFS="\t" FS="\t"  tmp2.conf > tmp1.conf
		awk 'NR==6{$(NF-8) = "'${cut2}'"}1' OFS="\t" FS="\t" tmp1.conf > tmp2.conf
		cp tmp2.conf ConfigFiles/OffsetAndCoverage_Analyzer_N0V2014_H4_AboveRun1924.conf 
		#sleep 5
